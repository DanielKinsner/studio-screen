...
- [ ] Implement, run `npx vitest run src/spring.test.ts`, commit.
...
- [ ] Tests: jittery input (±0.004 noise around a line) comes out with < 40% of the raw point-to-point jitter; at each click time the smoothed position equals the click position to 1e-6; `cursorOpacity` hides after 1.5 s idle and respects `hiddenCursor`; querying 500 random times gives identical results to querying them in ascending order; `cursorSmoothing: 0` returns raw positions.
- [ ] Replace the per-frame scans in `compositor.ts` (`p.points.find` for clicks, `p.points.filter` for shortcuts) with `clickAt`/`shortcutAt`.
- [ ] Update the old "smooths pointer movement" test to the new semantics, run all unit tests + build, commit.
...
- [ ] Tests:
  ...
  - no jumps: sampling at 1/60 s, |Δscale| < 0.08 per frame and |Δ²scale| < 0.02;
...
- [ ] Wire `compositor.ts` to the new `cameraAt`/`poseAt`, remove `zoomWeight` usage, run unit tests + build, commit.
...
- [ ] Tests: `migrateProject` of a project saved with `motionEase: "gentle"` and no `cameraResponse` gets Floaty values; `cleanSettings` clamps `cameraResponse`/`cameraBounce`; built-in and saved looks that only carry `motionEase` apply matching response/bounce.
- [ ] UI: "Camera feel" three-button group (Snappy / Smooth / Floaty) replaces the Movement select; Advanced → "Speed" (response) and "Bounce" sliders, which switch the group to Custom; "Follow cursor while zoomed" toggle; a note on zooms with click keyframes ("Follows N clicks. Moving the focus point pins it to one spot.") and moving Focus X/Y clears `focus`.
- [ ] Update Playwright tests that referenced the old Movement select, run unit tests + build + `node tests/browser-smoke.mjs` + `node tests/v2-proof.mjs`, commit.

### Task 6: Playback that holds 60 fps

**Files:** Modify `src/App.tsx`; create `tests/a1-preview-perf.mjs`.

- [ ] Playback `tick` renders straight to the canvas (no React state per frame); playhead and timecode update through refs; React `time` state is refreshed at ~4 Hz and on pause.
- [ ] While a source video is playing, the preview time comes from `video.currentTime` (the video clock) so the drawn cursor stays locked to the footage; wall clock only for the procedural sample.
- [ ] Perf test: builds a 10-minute synthetic project (FFmpeg `testsrc2` 1280×720 30 fps video, 36,000 pointer samples, a click every 5 s, 3D mode, motion blur 25) and imports it; plays 8 s; passes when p95 frame interval ≤ 20 ms and < 2% of intervals exceed 34 ms. Writes `tests/a1-perf-results.json`.
- [ ] Run everything, commit.

### Task 7: Phase close

- [ ] Run `npm run test`, `npm run build`, `node tests/browser-smoke.mjs`, `node tests/v2-proof.mjs`, `node tests/v2-visual.mjs`, `node tests/a1-preview-perf.mjs`.
- [ ] Update README (structure + behaviour), FEATURE-PARITY.md rows (auto zoom, motion styles, cursor smoothing), VALIDATION.md (A1 evidence), STATUS.md, and write Dan's hand-test script.
- [ ] Commit and push.
