...
`npm ci` / `npm install` must print `mediabunny@1.56.2 ✔`: that patch (`patches/`) fixes export timing for helper recordings, and a build without it exports stalls. The native capability command prints individual checks; inspect each result, not just its exit code. If the helper is unavailable, capture can fall back to browser capture, which has different cursor behavior.
...
The current package version is **0.4.3**; the app's footer reads it from `package.json` at build time. The office PC's `release/Cool Story 0.4.3.exe` is 103,215,321 bytes, SHA-256 `E88C635805E64736644B45F1D53342F82B5440745109CD47503C0746ED53A966` (default compression; the 130 MB 0.4.0 used level 3); the installed copy in `%LOCALAPPDATA%\Programs\Cool Story` is identical. The previous `release/Studio Screen 0.4.2.exe` is 103,250,845 bytes, SHA-256 `4292B8F40722DDE9CA4AD88D210C6D1AC8B9FCD04FF38D92431BBDEDE89B41B7`. Check it with `node tests/packaged-smoke.mjs --portable` (uses a throwaway profile). Do not identify an old executable solely by its filename; source and the current status document take precedence over old artifact receipts.
