...
**Idle-PC run, 2026-09-22 12:44–12:54** (details in VALIDATION.md): `a4-cadence` passed (first green under its final rule), `a2-recording-ui`, `a5-open-speed`, `desktop-capture`, `export-formats`, `audio-proof`, `alt-tilt`, `export-location`, `hidpi-window` passed; `a4-av-sync` 70 ms, unchanged from 0.4.2 (fails its rule by design on this PC). **Second idle window, 14:05–14:38:** `a4-native-capture` passes 20 of 21 checks after its fixture-on-top fix (Ctrl+K named, typing anonymous, 0.5 px clicks, crash recovery); its one failure is the same 66 ms playback offset. **`a4-soak` passed**: 30 minutes at 4K60, memory flat at 118–130 MB, 108,281 frames with 0 dropped, audio and video ending on the same millisecond.
...
4. Still unverified: a timed 4K60 export (no test exists yet). The 30-minute 4K soak passed on 0.4.3 (VALIDATION.md).
