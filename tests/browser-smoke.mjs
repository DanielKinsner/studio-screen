...
await page.getByRole('button', { name: 'Sage background', exact: true }).click();
...
await page.getByLabel('Annotation text', { exact: true }).fill('A working screen editor');
...
const file = await downloaded; await file.saveAs('tests/sample-export.webm');
await page.getByText('That’s a wrap.', { exact: true }).waitFor();
await page.getByRole('button', { name: 'Close dialog', exact: true }).click();
await page.getByText('Saved locally', { exact: true }).waitFor();
await page.reload();
await page.getByText('Saved locally', { exact: true }).waitFor();
if (await page.getByLabel('Trim end in seconds').inputValue() !== '3') throw new Error('Saved edits did not survive reload');
...
await fs.writeFile('tests/browser-results.json', JSON.stringify({ errors, persistence: true, exportBytes: (await fs.stat('tests/sample-export.webm')).size, mobileOverflow: false }, null, 2));
if (errors.length) throw new Error(errors.join('\n'));
await browser.close();
console.log('PASS: editor controls, real WebM export, persistence, mobile viewport, no page errors.');
